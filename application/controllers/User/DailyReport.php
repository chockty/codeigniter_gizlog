...
class DailyReport extends CI_Controller {
		...
		public function edit($daily_report_id)
		{
				$this->load->helper('date');
				$this->load->helper('typography');
				...
				$this->load->view('common/user');
				$this->load->view('user/daily_report/edit', $data);
		}

		public function update($daily_report_id)
		{
				$this->load->helper('date');
				...
				if ($this->form_validation->run() === FALSE) {
					...
					$this->daily_report_model->update_entry($daily_report_id);
					return 'success';
					// $this->load->view('common/user');
					// $this->load->view('user/index');
				}
